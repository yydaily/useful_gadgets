...
### 所有事项的位置
todo_dir_path = os.path.abspath(__file__)
todo_dir_path = todo_dir_path[:len(todo_dir_path)-10] + 'todo_info/'
### 删除无效的行 
def init(path):
	real_path = todo_dir_path + path
	with open(real_path, 'r') as r:
		lines = r.readlines()
		r.close()
	with open(real_path, 'w') as w:
		for l in lines:
			if len(l) <= 2:
				continue
			w.write(l)
...
# todo rn todo 1 XX
	if len(sys.argv) == 2:
		print('tell me to rename what')
		return 
	if len(sys.argv) == 3:
		print('tell me which tag to rename')
		return
	if len(sys.argv) == 4:
		print('tell me what you want it rename to')
		return
	from_file_path = todo_dir_path + sys.argv[2]
	row_ind = int(sys.argv[3])
	...
	with open(from_file_path, 'r') as f:
		...
		content = lines[row_ind-1]
		if new_content[0] == '-':
			new_content = str(content) + new_content
			new_content = new_content.replace('\n', '')
		print('rename this content:', content, ' -> ', new_content)
		f.close()
	del_from_file(from_file_path, content)
	save_to_file(from_file_path, new_content)
...
if __name__ == '__main__':
	...
	if sys.argv[1] == 'add':
		...
		del_todo()
	elif sys.argv[1] == 'rename' or sys.argv[1] == 'rn':
		rename()
	else:
